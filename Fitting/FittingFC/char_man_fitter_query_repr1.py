...
class CharManFitterQueryRepr1(MultiLevelAttentionCompositeFitter):
    ...
    def fit(self,
        ...
        for epoch_num in range(self._n_iter):
            ...
            epoch_loss, epoch_ce_loss, epoch_con_loss1 = 0.0, 0.0, 0.0 
            batch_cnt = 0
            ...
            for (minibatch_num,
                ...
                if self._loss in ["bpr", "hinge", "pce", "bce", "cross_entropy",
                    ...
                    loss, (ce_loss, con_loss1) = self._get_multiple_evidences_predictions_normal(
    ...
    def _get_multiple_evidences_predictions_normal(self, query_ids: torch.Tensor,
        ...
        # (B,)
        predictions, repr1, noise_pred2, noise_repr2 = self._net(query_contents, evd_doc_contents, **additional_paramters)
        ...
        loss =  ce_loss + self.con_lambda * con_loss_1 
       
        return  loss, (ce_loss.item(), con_loss_1.item())
    ...
    def _mask_token(self, x: torch.Tensor):
        ...
        drop_mask = torch.empty(
            (x.size(1), ),
            dtype=torch.float32,
            device=x.device).uniform_(0, 1) < self.mask_rate
        x = x.clone()
